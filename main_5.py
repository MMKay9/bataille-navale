...
class BatailleNavaleApp(tk.Tk):
    def __init__(self):
        ...
        # Représentations internes des plateaux 
        # 0 = vide, 1 = navire, 2 = tir raté, 3 = tir touché
        ...
        # Détails de navires (player / computer) : 
        # liste de dicos {name, size, coordinates, hits, sunk}
    ...
    def player_shoot_computer(self, row, col):
        ...
        # Sinon, l’ordinateur tire à son tour
    ...
    def update_ship_hit(self, ships_details, row, col, is_computer=False):
        ...
        et si c’est un navire de l’ordi coulé, on affiche visuellement son emplacement complet.
        """
        for ship in ships_details:
            if (row, col) in ship['coordinates']:
                ...
                if ship['hits'] == ship['size']:
                    ...
                    # Si c'est un navire de l'ordinateur, on l'affiche visuellement sur la grille
                    if is_computer:
                        self.reveal_sunk_ship(ship)
    ...
    def reveal_sunk_ship(self, ship):
        """
        Affiche visuellement TOUTES les cases du navire coulé sur la grille de l’ordinateur 
        (même celles qui n’ont pas été touchées).
        Exemple : on colorie en gris foncé et on place un "X" blanc.
        """
        for (r, c) in ship['coordinates']:
            self.computer_buttons[r][c].configure(
                bg="dim gray",    # Couleur de fond
                fg="white",       # Couleur du texte
                text="X"          # On peut mettre "X", "S", ou tout autre symbole
            )
